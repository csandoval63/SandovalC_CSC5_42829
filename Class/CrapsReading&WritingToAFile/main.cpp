...
{
    //set random number seed for variablitily/different numbers, and open a file
        
    srand(static_cast<unsigned int>(time(0)));
    ofstream out;//Output / out used can be anything but its used for file
    ifstream in;//Input /used for reading files
    ...
    in.open("inCrap.dat");
    in >> nGames;//inputs the value from the file to the program
    ...
    //Exit Stage Right!
    out.close();
    in.close();//make sure to close both
